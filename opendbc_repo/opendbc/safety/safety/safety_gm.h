...
#define GM_COMMON_RX_CHECKS \
    ...
    {.msg = {{0x1E1, 0, 7, .ignore_checksum = true, .ignore_counter = true, .frequency = 10U}, \
             {0x1E1, 2, 7, .ignore_checksum = true, .ignore_counter = true, .frequency = 0U}}}, \
...
static void gm_rx_hook(const CANPacket_t *to_push) {
  if (GET_BUS(to_push) == 0U) {
    ...
    if (addr == 0xC9) {
      if (gm_force_brake_c9 && (gm_hw == GM_CAM)) {
        brake_pressed = (GET_BYTE(to_push, 5) & 0x01U) != 0U;
      }
      acc_main_on = (GET_BYTE(to_push, 3) & 0x20U) != 0U;
    } else if ((addr == 0xBE) && ((gm_hw == GM_ASCM) || (gm_hw == GM_SDGM)) && !gm_force_brake_c9) {
...
static safety_config gm_init(uint16_t param) {
  ...
  static const CanMsg GM_CAM_LONG_TX_MSGS[] = {{0x180, 0, 4}, {0x315, 0, 5}, {0x2CB, 0, 8}, {0x370, 0, 6}, {0x200, 0, 6}, {0xBD, 0, 7}, {0x1F5, 0, 8},   // pt bus
                                              {0x184, 2, 8}, {0x315, 2, 5}, {0x1E1, 2, 7}};  // camera bus

  static const CanMsg GM_SDGM_TX_MSGS[] = {{0x180, 0, 4}, {0x315, 0, 5}, {0x2CB, 0, 8}, {0x370, 0, 6},  // pt bus
                                          {0x184, 2, 8}, {0x1E1, 2, 7}, {0x315, 2, 5}};  // camera bus
  // TODO: do checksum and counter checks. Add correct timestep, 0.1s for now.

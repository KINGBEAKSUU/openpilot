...
static safety_config gm_init(uint16_t param) {
  ...
  // FIX: ASCM_INT도 pcmCruise 환경으로 인정 (interface에서 ASCM_INT는 fwdCamera/pcmCruise 사용)
  gm_pcm_cruise = (((gm_hw == GM_CAM) || gm_ascm_int) && (!gm_cam_long || gm_cc_long) && !gm_force_ascm && !gm_pedal_long);

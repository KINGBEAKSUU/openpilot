...
class TestGmSdgmSafety(TestGmSafetyBase):
  ...
  def setUp(self):
    ...
    self.safety.set_safety_hooks(CarParams.SafetyModel.gm, GMSafetyFlags.HW_SDGM)
    self.safety.init_tests()
...
class TestGmInterceptorSafety(common.GasInterceptorSafetyTest, TestGmCameraSafety):
  ...
  def setUp(self):
    ...
    self.safety.set_safety_hooks(
      CarParams.SafetyModel.gm, GMSafetyFlags.HW_CAM | GMSafetyFlags.NO_ACC | GMSafetyFlags.PEDAL_LONG | GMSafetyFlags.GAS_INTERCEPTOR)
    self.safety.init_tests()
...
class TestGmCcLongitudinalSafety(TestGmCameraSafety):
  ...
  def setUp(self):
    ...
    self.packer_chassis = CANPackerPanda("gm_global_a_chassis")
    self.safety = libpanda_py.libpanda
    self.safety.set_safety_hooks(CarParams.SafetyModel.gm, GMSafetyFlags.HW_CAM | GMSafetyFlags.NO_ACC | GMSafetyFlags.CC_LONG)
    self.safety.init_tests()
